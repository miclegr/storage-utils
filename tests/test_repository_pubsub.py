...
@pytest.mark.asyncio
async def test_pull(fake_pubsub_subscriber_client, fake_pubsub_subscriber_buffer, fake_messages, fake_data):

    fake_pubsub_subscriber_buffer['test'] = fake_messages
    repository = PubSubRepository(fake_pubsub_subscriber_client(), {}, defaultdict(list), defaultdict(list))
...
async def test_push(fake_pubsub_subscriber_client, fake_data):
    ...
    repository = PubSubRepository(fake_pubsub_subscriber_client(), {}, defaultdict(list), publish_buffer)
