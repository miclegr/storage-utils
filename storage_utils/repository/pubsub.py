...
class PubSubRepository(Repository):
    ...
    async def _pull_from_subscription(self, subscription: str, MessageType: Parsable):
        ...
        for message in messages:
            ...
            output.append(message.to_domain())

        return output

    def _push_to_topic(self, topic:str, MessageType: Pushable, items: List[Any]):

        buffer = self.pubsub_publisher_buffer[topic]
        for item in items:
            buffer.append(MessageType.from_domain(item))
